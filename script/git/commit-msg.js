...
const msg = fs.readFileSync(`${process.env.PWD}/${process.argv[2]}`, 'utf-8').trim();

const commitRE = [/^((feat|fix|docs|style|refactor|perf|test|chore)(\(.+\))?:).{1,100}/, /^Merge.*/gm];

if (!commitRE.some((re) => re.test(msg))) {
  console.error(
    `  ${chalk.bgRed.white(' ERROR ')} ${chalk.red(`提交信息格式不正确`)}\n\n${chalk.red(
      ...
      `    ${chalk.red(`refactor：重构（即不是新增功能，也不是修改 bug 的代码变动）`)}\n` +
