...
import { isFunction, isInstanceOf, isArray, isObject, isEqual, isNull, isNone } from 'ts-fns';
...
export function instance(pattern) {
  const rule = new Rule({
    ...
    validate(data, key) {
      ...
      return isInstanceOf(value, pattern, true)
        ? null
...
export function equal(pattern) {
  const rule = new Rule({
    ...
    validate: (data, key) => {
      const value = data[key];
      return isEqual(value, pattern) ? null : new TyError({ type: 'exception', value, pattern, name: 'equal' });
    },
  });
  return rule;
}
