...
import { isFunction, isBoolean, isInstanceOf, isString, isNull, getConstructorOf } from 'ts-fns';
...
export class Rule {
   ...
   */
  constructor(options = {}) {
    const { name, pattern, message } = options;
  ...
  validate(data, key, pattern) {
    ...
    if (isInstanceOf(pattern, Rule)) {
      const rule =
        this.isStrict && !pattern.isStrict
          ? pattern.strict
          : !this.isStrict && this.isLoose && !pattern.isStrict && pattern.isLoose
          ? pattern.loose
          : pattern;
   ...
   */
    ...
    const { shouldcheck, use, decorate, override, complete } = this.options;
