...
export class Model {
  constructor(data = {}, options = {}) {
    ...
    const { parent, key: keyInParent, scenes = this._takeSceneCodes() } = options;
    ...
    const checkNeeds = () => {
      ...
      for (let i = 0, len = needs.length; i < len; i++) {
        ...
        throw new Error(
          `Dependence is not given, please check dependencies graph. [${JSON.stringify(
            need,
          )}] is needed by [${JSON.stringify(by)}] but not given in Model`,
        );
      }
    };
  ...
  init(data = {}) {
    ...
    const validatingQueues = {};
    const cachedErrorsUpdators = {};
      ...
      each(
        ...
        (descriptor, attr) => {
          ...
          if (get || set) {
            ...
            const get = () =>
              attach.call(this, key, attr).then((next) => {
                if (next === attrValue) {
                  return;
                }
            ...
            viewDef[attr] = {
              get: () => this.memo(getter, compare, depend),
              enumerable: true,
              configurable: true,
            };
          }
  ...
  collect(collector, inverse) {
    if (isFunction(collector)) {
      if (inverse === true) {
        // if true, do not collect deps, and return normal value of collector fn
        if (this.$collection) {
    ...
    const validate = (key, emit) => {
      ...
      const outs = decideby(() => {
        ...
        if (
          isArray(value) &&
          value?.length > 0 &&
          !value.some((item) => !isInstanceOf(item, Model)) &&
          !this.$schema.disabled(key, value, this)
        ) {
          const suberrs = value.map((model) => model.validate());
          suberrs.forEach((items, i) => {
            items.forEach((item) => {
              item.absKeyPath = [...this.$absKeyPath, key, i, item.key];
        ...
        if (value && isInstanceOf(value, Model) && !this.$schema.disabled(key, value, this)) {
          ...
          items.forEach((item) => {
            item.absKeyPath = [...this.$absKeyPath, key, item.key];
            item.key = makeKeyPath([...this.$absKeyPath, key, item.key]);
            outs.push(item);
          });
          return outs;
        }
        return outs?.map((item) => {
          return {
            ...item,
            absKeyPath: [...this.$absKeyPath, item.key],
          };
        });
      });
      ...
      const errors = [
        ...errs,
        ...outs?.map((out) => {
          // 原有key规则不统一，新加入一个absKey把整体路径返回
          const result = { ...out, absKey: out.absKeyPath?.join('.') };
          delete result.absKeyPath;
          return result;
        }),
      ];
      if (emit) {
  ...
  validateAsync(key) {
    ...
    const validate = (key, emit) => {
      ...
      const defer = decideby(() =>
        // check the given meta validators at first
        this.$schema.validateAsync(key, value, this).then((preouts) => {
          const outs = [...preouts];
          if (
            isArray(value) &&
            value?.length > 0 &&
  ...
  getData(key) {
    ...
    if (meta && !meta[ComputeSymbol] && meta.compute) {
