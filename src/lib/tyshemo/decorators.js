...
const createDecorator = (name, fn, force) => (protos, key, descriptor) => {
  ...
  if (force && !(protos instanceof Model)) {
    throw new Error(
      `TySheMo: @${name} only works on class which extends Model.`
    );
  }
  ...
  if (descriptor) {
    const oop = () => {
      throw new Error(
        `TySheMo: @${name} only works on a class property which has no initializer.`
      );
    };
  ...
  return createDecorator(
    ...
    (protos, key) => {
      ...
      if (isInheritedOf(entry, Meta) || isInstanceOf(entry, Meta)) {
        ...
        isInheritedOf(entry, Model) ||
        (isArray(entry) && !entry.some((item) => !isInheritedOf(item, Model)))
      ) {
...
export function state(options) {
  if (
    options &&
    (isInstanceOf(options, StateMeta) || isInheritedOf(options, StateMeta))
  ) {
    return meta(options);
  }
  if (
    options &&
    (isInstanceOf(options, SceneStateMeta) ||
      isInheritedOf(options, SceneStateMeta))
  ) {
    return meta(options);
  }
  ...
  return createDecorator(
    ...
    (protos, key) => {
      if (!('value' in options) && !options.get && !options.set) {
        throw new Error(
          `TySheMo: @state params should not contain either value or get/set.`
        );
      }
      if ('value' in options && (options.get || options.set)) {
        throw new Error(
          `TySheMo: @state params should not contain value and get/set together.`
        );
      }
