...
export function query(source, ...params) {
  ...
  if (type === SOURCE_TYPES.ACTION) {
    throw new Error(
      `[alegb]: action不能用在query中，只能用在request中，query只能使用source.`
    );
  }
}
...
function queryCompose(source, ...params) {
  ...
  const { deps } = item;
  item.deferer = Promise.all(
    deps.filter((dep) => dep.deferer).map((dep) => dep.deferer)
  ).then(() => item.value);
...
export function request(source, ...params) {
  ...
  throw new Error(
    `[alegb]: request只能使用action和原子source，不能使用复合source`
  );
}
