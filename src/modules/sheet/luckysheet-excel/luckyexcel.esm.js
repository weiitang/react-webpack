...
var WorkCellBase = /** @class */ (function () {
    function WorkCellBase() {
    }
    return WorkCellBase;
}());
var WorkCell = /** @class */ (function (_super) {
    __extends(WorkCell, _super);
    function WorkCell(data) {
        var _a, _b, _c;
        var _this = _super.call(this) || this;
        var r = data.r, c = data.c;
        var _d = data.v || {}, ct = _d.ct, v = _d.v, m = _d.m, f = _d.f, vt = _d.vt, ht = _d.ht, tb = _d.tb, tr = _d.tr, bg = _d.bg, fc = _d.fc, bl = _d.bl, it = _d.it, cl = _d.cl, un = _d.un, fs = _d.fs, ff = _d.ff;
        _this.key = XLSX.utils.encode_cell({ c: c, r: r });
        _this.data = { v: '', t: 's', s: {} };
        if (ct) {
            if (v) {
                // 错误时 值有特定mapping
                if (ErrorValueMap[v] !== undefined) {
                    _this.data.v = ErrorValueMap[v];
                }
                else {
                    _this.data.v = v;
                }
            ...
            else if (Array.isArray(ct === null || ct === void 0 ? void 0 : ct.s)) {
                // inline string格式的简易处理
                _this.data.t = 's';
                _this.data.v = (_a = ct === null || ct === void 0 ? void 0 : ct.s) === null || _a === void 0 ? void 0 : _a.reduce(function (prev, cur) { return prev + cur.v; }, '');
            }
            // 数字 & 错误
            if ((ct === null || ct === void 0 ? void 0 : ct.t) === 'n' && (ct === null || ct === void 0 ? void 0 : ct.t) === 'e') {
                _this.data.t = ct.t;
                _this.data.z = ct.fa;
                f && (_this.data.f = f);
            }
        ...
        // 对齐方式 换行 文字方向
        if ([vt, ht, tb, tr].some(function (item) { return item !== undefined; })) {
            _this.data.s.alignment = {};
            vt !== undefined && (_this.data.s.alignment.vertical = verticalMap[vt]);
            ht !== undefined && (_this.data.s.alignment.horizontal = horizontalMap[ht]);
            tb !== undefined && (_this.data.s.alignment.wrapText = wrapTextMap[tb]);
            tr !== undefined && (_this.data.s.alignment.textRotation = textRotationMap[tr]);
        }
        // 背景色
        if (bg) {
            _this.data.s.fill = {
                fgColor: { rgb: ((_b = rgbToHex(bg)) === null || _b === void 0 ? void 0 : _b.replace('#', '')) || 'ffffff' }
            };
        }
        // 文本样式
        if ([fc, bl, it, cl, un, fs].some(function (item) { return item !== undefined; })) {
            _this.data.s.font = {};
            fc !== undefined && (_this.data.s.font.color = { rgb: ((_c = rgbToHex(fc)) === null || _c === void 0 ? void 0 : _c.replace('#', '')) || '000000' });
...
var BorderInfoBase = /** @class */ (function () {
    function BorderInfoBase() {
    }
    return BorderInfoBase;
}());
var BorderInfo = /** @class */ (function (_super) {
    ...
    function BorderInfo(data) {
        ...
        data === null || data === void 0 ? void 0 : data.forEach(function (item) {
            ...
            if (rangeType === 'cell') {
                ...
                var cellKey_1 = XLSX.utils.encode_cell({ c: value_1.col_index, r: value_1.row_index });
                _this.data[cellKey_1] = {};
                ['t', 'b', 'l', 'r'].forEach(function (p) {
                    var _a;
                    var borderStyle = value_1[p];
                    if (borderStyle && excelBorderPositions[p]) {
                        _this.data[cellKey_1][excelBorderPositions[p]] = {
                            style: excelBorderStyles[borderStyle.style] || 'thin',
                            color: { rgb: ((_a = rgbToHex(borderStyle.color)) === null || _a === void 0 ? void 0 : _a.replace('#', '')) || '000000' }
                        };
                    }
                });
            }
            ...
            if (rangeType === 'range') {
                var color = item.color, style = item.style, _b = item.range, range = _b === void 0 ? [] : _b, borderType = item.borderType;
                var borderStyle = {
                    style: excelBorderStyles[style] || 'thin',
                    color: { rgb: ((_a = rgbToHex(color)) === null || _a === void 0 ? void 0 : _a.replace('#', '')) || '000000' }
                };
                for (var _i = 0, range_1 = range; _i < range_1.length; _i++) {
                    var rangeItem = range_1[_i];
                    var _c = rangeItem.row, rangeStartRow = _c[0], rangeEndRow = _c[1];
                    var _d = rangeItem.column, rangeStartCol = _d[0], rangeEndCol = _d[1];
                    for (var row = rangeStartRow; row <= rangeEndRow; row++) {
                        for (var col = rangeStartCol; col <= rangeEndCol; col++) {
                            var cellKey = XLSX.utils.encode_cell({ c: col, r: row });
                            _this.data[cellKey] = {};
                            // 检查上下左右边的情况，按需添加边框
                            if (borderType === 'border-none') {
                                ...
                                var isOnTop = isOnTopBorder({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnBottom = isOnBottomBorder({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnRight = isOnRightBorder({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnLeft = isOnLeftBorder({ row: row, column: col }, rangeItem.row, rangeItem.column);
...
function isOnTopBorder(cell, rowRange, columnRange) {
    ...
    var rangeStartRow = rowRange[0], rangeEndRow = rowRange[1];
    var rangeStartCol = columnRange[0], rangeEndCol = columnRange[1];
...
var WorkSheetBase = /** @class */ (function () {
...
var WorkSheet = /** @class */ (function (_super) {
    ...
    function WorkSheet(sheetData) {
        var _this = _super.call(this) || this;
        var config = sheetData.config, column = sheetData.column, row = sheetData.row, celldata = sheetData.celldata, name = sheetData.name, data = sheetData.data;
        var _a = config || {}, columnlen = _a.columnlen, rowlen = _a.rowlen, merge = _a.merge, borderInfo = _a.borderInfo;
        _this.data = {};
        _this.border = {};
        _this.name = name;
        // 解析borderInfo
        var border = new BorderInfo(borderInfo);
        for (var _i = 0, _b = Object.entries(border.data); _i < _b.length; _i++) {
            var _c = _b[_i], key = _c[0], borderValue = _c[1];
            _this.data[key] = {};
        ...
        celldata === null || celldata === void 0 ? void 0 : celldata.forEach(function (data) {
            var _a;
            var cell = new WorkCell(data);
            if (_this.border[cell.key]) {
                _this.data[cell.key] = __assign(__assign({}, (cell.toData() || {})), { s: __assign(__assign({}, (((_a = cell.toData()) === null || _a === void 0 ? void 0 : _a.s) || {})), { border: _this.border[cell.key] || {} }) });
            }
            else {
        ...
        // 规定数据范围
        var endCellRange = XLSX.utils.encode_cell({ c: column - 1, r: row - 1 });
        _this.ref = "A1:" + endCellRange;
        // 规定行列宽高
        _this.cols = [];
        if (columnlen) {
            Object.keys(columnlen).forEach(function (index) {
                _this.cols[Number(index)] = { wpx: Math.round(parseFloat(columnlen[index]) * 0.75) };
            });
        }
        _this.rows = [];
        if (rowlen) {
            Object.keys(rowlen).forEach(function (index) {
                _this.rows[Number(index)] = { hpx: Math.round(parseFloat(rowlen[index]) * 0.75) };
            });
        }
        // 合并单元格范围
        _this.merges = [];
        if (merge) {
            Object.keys(merge).forEach(function (key) {
                // rs cs 为所占行数
                var _a = merge[key], r = _a.r, c = _a.c, rs = _a.rs, cs = _a.cs;
                // s为合并单元格开始行列  e为结束
                var mergeValue = {
                    s: { r: r, c: c },
                    e: { r: r + (rs - 1), c: c + (cs - 1) }
...
var ExcelFileBase = /** @class */ (function () {
    function ExcelFileBase() {
    }
    return ExcelFileBase;
}());
var ExcelFile = /** @class */ (function (_super) {
    ...
    function ExcelFile(luckysheetJson, fileName) {
        var _this = _super.call(this) || this;
        var title = luckysheetJson.title, data = luckysheetJson.data;
        _this.fileName = fileName || luckysheetJson.title;
        _this.sheets = data === null || data === void 0 ? void 0 : data.map(function (item) { return new WorkSheet(item); });
        return _this;
    }
...
var WorkCellBase$1 = /** @class */ (function () {
    function WorkCellBase() {
    }
    return WorkCellBase;
}());
var WorkCell$1 = /** @class */ (function (_super) {
    __extends(WorkCell, _super);
    function WorkCell(worksheet, data) {
        var _this = _super.call(this) || this;
        var r = data.r, c = data.c;
        _this.key = encodeCell({ c: c, r: r });
        _this.worksheet = worksheet;
        _this.cell = worksheet.worksheet.getCell(_this.key);
        _this._parseValue(data);
        _this.cell.font = _this._parseFont(data === null || data === void 0 ? void 0 : data.v);
        _this._parseStyle(data === null || data === void 0 ? void 0 : data.v);
        return _this;
    }
    WorkCell.prototype._parseValue = function (data) {
        var _this = this;
        var _a, _b, _c, _d, _e;
        var r = data.r, c = data.c;
        if ((data === null || data === void 0 ? void 0 : data.v) === undefined) {
            return;
        }
        if (typeof (data === null || data === void 0 ? void 0 : data.v) === 'string' || (data === null || data === void 0 ? void 0 : data.v) === null) {
            this.cell.value = data.v;
            return;
        }
        var _f = data.v, ct = _f.ct, v = _f.v, f = _f.f;
        if (ct) {
            // 自动格式、数字、字符串、布尔值 直接赋值
            if (['g', 'n', 's', 'b'].includes(ct === null || ct === void 0 ? void 0 : ct.t)) {
                this.cell.value = v;
            ...
            if ((ct === null || ct === void 0 ? void 0 : ct.t) === 'd') {
                ...
                var utcOffset = date0.getTimezoneOffset();
                var cellValue = new Date(0, 0, parseInt(v) - 1, 0, -utcOffset, 0);
            ...
            // 富文本
            if ((ct === null || ct === void 0 ? void 0 : ct.t) === 'inlineStr' && ((_a = ct === null || ct === void 0 ? void 0 : ct.s) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                this.cell.value = {
                    richText: (_b = ct === null || ct === void 0 ? void 0 : ct.s) === null || _b === void 0 ? void 0 : _b.map(function (item) {
                        return {
                            text: item.v,
                            font: _this._parseFont(item)
                        };
                    })
                };
            }
            // // 错误值
            if ((ct === null || ct === void 0 ? void 0 : ct.t) === 'e' && !f) {
                this.cell.value = { error: ErrorValueMap$1[v] || Excel.ErrorValue.Value };
            }
        }
        // 超链接
        if ((_c = this.worksheet.hyperlink) === null || _c === void 0 ? void 0 : _c[r + "_" + c]) {
            var hyperlink = (_d = this.worksheet.hyperlink) === null || _d === void 0 ? void 0 : _d[r + "_" + c];
            var linkAddress = hyperlink.linkAddress;
            if (hyperlink.linkType === 'internal') {
                var _g = (_e = hyperlink.linkAddress) === null || _e === void 0 ? void 0 : _e.split('!'), sheet = _g[0], cell = _g[1];
    ...
    // 格式化字体样式
    WorkCell.prototype._parseFont = function (data) {
        var _a;
        if (typeof data === 'string' || data === undefined) {
            return {};
        }
        var _b = data || {}, fc = _b.fc, bl = _b.bl, it = _b.it, cl = _b.cl, un = _b.un, fs = _b.fs, ff = _b.ff;
        return {
            name: FontFamilyMap[ff] || ff,
            size: fs !== null && fs !== void 0 ? fs : 10,
            family: 1,
            color: { argb: ((_a = rgbToHex(fc)) === null || _a === void 0 ? void 0 : _a.replace('#', '')) || '000000' },
            bold: !!bl,
            italic: !!it,
            underline: !!un,
            strike: !!cl,
    ...
    WorkCell.prototype._parseStyle = function (data) {
        var _a, _b;
        if (typeof data === 'string' || data === undefined) {
            return {};
        }
        var _c = data || {}, vt = _c.vt, ht = _c.ht, tb = _c.tb, tr = _c.tr, ti = _c.ti, bg = _c.bg;
        // 对齐
        var alignment = {};
        alignment.vertical = verticalMap$1[vt] || 'middle';
        alignment.horizontal = horizontalMap$1[ht] || 'left';
        tb !== undefined && (alignment.wrapText = wrapTextMap$1[tb]);
        ti !== undefined && (alignment.indent = ti !== null && ti !== void 0 ? ti : 0);
        tr !== undefined && (alignment.textRotation = (_a = textRotationMap$1[tr]) !== null && _a !== void 0 ? _a : 0);
        ...
        if (bg) {
            this.cell.fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: ((_b = rgbToHex(bg)) === null || _b === void 0 ? void 0 : _b.replace('#', '')) || 'ffffff' }
            };
        }
    };
    WorkCell.prototype.toData = function () {
...
var BorderInfoBase$1 = /** @class */ (function () {
    function BorderInfoBase() {
    }
    return BorderInfoBase;
}());
var BorderInfo$1 = /** @class */ (function (_super) {
    __extends(BorderInfo, _super);
    function BorderInfo(data) {
        var _this = _super.call(this) || this;
        _this.data = {};
        data === null || data === void 0 ? void 0 : data.forEach(function (item) {
            ...
            if (rangeType === 'cell') {
                ...
                ['t', 'b', 'l', 'r'].forEach(function (p) {
                    ...
                    if (borderStyle && excelBorderPositions[p]) {
                        _this.data[cellKey_1][excelBorderPositions[p]] = {
                            style: excelBorderStyles$1[borderStyle.style] || 'thin',
                            color: { argb: ((_a = rgbToHex(borderStyle.color)) === null || _a === void 0 ? void 0 : _a.replace('#', '')) || '000000' }
                        };
                    }
                });
            }
            ...
            if (rangeType === 'range') {
                var color = item.color, style = item.style, _b = item.range, range = _b === void 0 ? [] : _b, borderType = item.borderType;
                var borderStyle = {
                    style: excelBorderStyles$1[style] || 'thin',
                    color: { argb: ((_a = rgbToHex(color)) === null || _a === void 0 ? void 0 : _a.replace('#', '')) || '000000' }
                };
                for (var _i = 0, range_1 = range; _i < range_1.length; _i++) {
                    var rangeItem = range_1[_i];
                    var _c = rangeItem.row, rangeStartRow = _c[0], rangeEndRow = _c[1];
                    ...
                    for (var row = rangeStartRow; row <= rangeEndRow; row++) {
                        for (var col = rangeStartCol; col <= rangeEndCol; col++) {
                            ...
                            if (borderType === 'border-none') {
                                ...
                                var isOnTop = isOnTopBorder$1({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnBottom = isOnBottomBorder$1({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnRight = isOnRightBorder$1({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                var isOnLeft = isOnLeftBorder$1({ row: row, column: col }, rangeItem.row, rangeItem.column);
                                if (['border-top', 'border-outside'].includes(borderType) && isOnTop) {
                                    _this.data[cellKey].top = borderStyle;
                                }
                                if (['border-bottom', 'border-outside'].includes(borderType) && isOnBottom) {
...
function isOnTopBorder$1(cell, rowRange, columnRange) {
    var cellRow = cell.row, cellCol = cell.column;
    var rangeStartRow = rowRange[0], rangeEndRow = rowRange[1];
    var rangeStartCol = columnRange[0], rangeEndCol = columnRange[1];
    return cellRow === rangeStartRow;
}
function isOnBottomBorder$1(cell, rowRange, columnRange) {
    var cellRow = cell.row, cellCol = cell.column;
    var rangeStartRow = rowRange[0], rangeEndRow = rowRange[1];
    var rangeStartCol = columnRange[0], rangeEndCol = columnRange[1];
    return cellRow === rangeEndRow;
}
function isOnRightBorder$1(cell, rowRange, columnRange) {
    var cellRow = cell.row, cellCol = cell.column;
    var rangeStartRow = rowRange[0], rangeEndRow = rowRange[1];
    var rangeStartCol = columnRange[0], rangeEndCol = columnRange[1];
    return cellCol === rangeEndCol;
}
function isOnLeftBorder$1(cell, rowRange, columnRange) {
    var cellRow = cell.row, cellCol = cell.column;
    var rangeStartRow = rowRange[0], rangeEndRow = rowRange[1];
    var rangeStartCol = columnRange[0], rangeEndCol = columnRange[1];
    return cellCol === rangeStartCol;
...
var WorkSheetBase$1 = /** @class */ (function () {
    function WorkSheetBase() {
    }
    return WorkSheetBase;
}());
var WorkSheet$1 = /** @class */ (function (_super) {
    __extends(WorkSheet, _super);
    function WorkSheet(workbook, sheetData) {
        var _this = _super.call(this) || this;
        var config = sheetData.config, maxCol = sheetData.column, maxRow = sheetData.row, hyperlink = sheetData.hyperlink, celldata = sheetData.celldata, name = sheetData.name, data = sheetData.data, hide = sheetData.hide, defaultColWidth = sheetData.defaultColWidth, defaultRowHeight = sheetData.defaultRowHeight, color = sheetData.color;
        var _a = config || {}, columnlen = _a.columnlen, rowlen = _a.rowlen, merge = _a.merge, borderInfo = _a.borderInfo;
        _this.data = {};
        _this.border = {};
        _this.hyperlink = hyperlink;
        _this.name = name;
        _this.worksheet = workbook.addWorksheet(name, {
        ...
        // 解析数据
        celldata === null || celldata === void 0 ? void 0 : celldata.forEach(function (data) {
            var cell = new WorkCell$1(_this, data);
            _this.data[cell.key] = cell.cell;
        });
        // 规定行列宽高
        if (columnlen) {
            Object.keys(columnlen).forEach(function (_col) {
                var _a;
                if (parseInt(_col) > maxCol) {
                    return;
                }
                var colKey = chatatABC(parseInt(_col));
                var col = _this.worksheet.getColumn(colKey);
                col.width = Math.round(parseFloat((_a = columnlen === null || columnlen === void 0 ? void 0 : columnlen[_col]) !== null && _a !== void 0 ? _a : 70) * 0.17);
            });
        }
        if (rowlen) {
            Object.keys(rowlen).forEach(function (_row) {
                var _a;
                if (parseInt(_row) > maxRow) {
                    return;
                }
                var row = _this.worksheet.getRow(parseInt(_row) + 1);
                row.height = Math.round(parseFloat((_a = rowlen === null || rowlen === void 0 ? void 0 : rowlen[_row]) !== null && _a !== void 0 ? _a : 20) * 0.5);
            });
        }
        // 合并单元格
        if (merge) {
            Object.keys(merge).forEach(function (key) {
                ...
                var _a = merge[key], r = _a.r, c = _a.c, rs = _a.rs, cs = _a.cs;
                // s为扩展的offset
                var range = encodeRange({ r: r, c: c }, { r: r + rs - 1, c: c + cs - 1 });
                _this.worksheet.mergeCells(range);
            });
        }
        // 解析borderInfo
        var border = new BorderInfo$1(borderInfo);
        for (var _i = 0, _b = Object.entries(border.data); _i < _b.length; _i++) {
            var _c = _b[_i], key = _c[0], borderValue = _c[1];
            _this.border[key] = borderValue;
            _this.worksheet.getCell(key).border = borderValue;
        }
        return _this;
    }
    WorkSheet.prototype.toData = function () {
...
var ExcelFileBase$1 = /** @class */ (function () {
    function ExcelFileBase() {
    }
    return ExcelFileBase;
}());
var ExcelFile$1 = /** @class */ (function (_super) {
    __extends(ExcelFile, _super);
    /**
     * @param luckysheetJson luckysheet.toJson() 返回数据
     * @param fileName
    ...
    function ExcelFile(luckysheetJson, fileName) {
        ...
        var title = luckysheetJson.title, data = luckysheetJson.data;
        _this.fileName = fileName || luckysheetJson.title;
        _this.workbook = new Excel.Workbook();
        _this.sheets = data === null || data === void 0 ? void 0 : data.map(function (item) { return new WorkSheet$1(_this.workbook, item); });
        return _this;
    }
    ExcelFile.prototype.export = function () {
        return __awaiter(this, void 0, void 0, function () {
            var buffer, blob, url, link;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.workbook.xlsx.writeBuffer()];
                    case 1:
                        buffer = _a.sent();
...
var LuckyExcel = /** @class */ (function () {
    ...
    LuckyExcel.readExcel = function (data, callBack, errorHandler) {
        var workbook = XLSX.read(data, { bookDeps: true, cellStyles: true, raw: true, cellNF: true });
    };
    LuckyExcel.transformLuckyToExcel = function (luckysheetJson, callBack, errorHandler) {
        try {
            var excelFile = new ExcelFile(luckysheetJson);
            excelFile.export();
    ...
    LuckyExcel.transformLuckyToExceljs = function (luckysheetJson, callBack, errorHandler) {
        var excelFile = new ExcelFile$1(luckysheetJson);
        excelFile.export().then(function (data) {
...
//# sourceMappingURL=luckyexcel.esm.js.map
