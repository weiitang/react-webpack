...
import {
  formatNumberBySeparator,
  clearNumberZero,
} from '@src/utils/functions/number';
import type { IObj } from '@typings/utils';
import { isNumeric, isNumber } from 'ts-fns';
...
export function useInputNumber<T extends IObj = IObj>(
  ...
  const formatNumber = (num): string => {
    ...
    return formatNumberBySeparator(num, separator, 3);
  };
  ...
  const onTextChange = (nextText) => {
    ...
    // 全部选中后输入了一个值
    else if (nextText.length === 1 && isNumeric(nextText)) {
      setText(nextText);
      setNum(nextText);
      setCurr(1);
    }
    ...
    // 在末尾删除一位后留下小数点
      ...
      nextText[nextText.length - 1] === '.' &&
      text.indexOf(nextText) === 0
    ) {
      setText(nextText);
      const nextNum = toNum(nextText);
      setNum(nextNum);
